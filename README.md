...
Сначала откройте **Настройки → Инструменты разработчика → Состояния** и
скопируйте точный идентификатор основной сущности Tion 4S. Затем добавьте:

```yaml
yandex_smart_home:
  entity_config:
    fan.tion_breezer_4s_1:  # замените на свой реальный entity_id
      name: Бризер
      type: ventilation
      modes:
        fan_speed:
...
> разработчика. Его нужно сохранить в файле `configuration.yaml`. Если раздел
> `yandex_smart_home:` уже существует, добавьте сущность внутрь существующего
> `entity_config`, не создавая второй одноимённый раздел.

После сохранения проверьте конфигурацию и перезапустите Home Assistant. Затем
удалите старую карточку вентилятора из приложения **Дом с Алисой** и выполните
**Yaha Cloud → Обновить список устройств**.
